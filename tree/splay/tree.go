package splay

import (
	"fmt"
	"github.com/nnhatnam/skale"
)

type Iterator[T any] func(item T) bool

type Node[T any] struct {
	Value       T // value stored in the node
	Left, Right *Node[T]
}

func (n *Node[T]) String() string {
	return fmt.Sprintf("%v", n.Value)
}

func newNode[T any](value T) *Node[T] {
	return &Node[T]{Value: value}
}

type Splay[T any] struct {
	root  *Node[T]
	count int
	less  skale.LessFunc[T]
}

func New[T any](less skale.LessFunc[T]) *Splay[T] {
	return &Splay[T]{less: less}
}

func NewOrdered[T skale.Ordered]() *Splay[T] {
	return New(skale.Less[T]())
}

// -----------------------------------------internal methods-----------------------------------------

// rotateLeft rotates the node n to the left, and swap the color between the old root (n) and the new root (r)
func rotateLeft[T any](n *Node[T]) *Node[T] {
	r := n.Right
	n.Right = r.Left
	r.Left = n
	return r
}

// rotateRight rotates the node n to the right, and swap the color between the old root (n) and the new root (l)
func rotateRight[T any](n *Node[T]) *Node[T] {
	l := n.Left
	n.Left = l.Right
	l.Right = n
	return l

}

// min returns the minimum node in the tree with root n
func findMin[T any](n *Node[T]) *Node[T] {
	for n.Left != nil {
		n = n.Left
	}

	return n
}

// max return the maximum node in the tree with root n
func findMax[T any](n *Node[T]) *Node[T] {
	for n.Right != nil {
		n = n.Right
	}

	return n
}

func inOrder[T any](n *Node[T], iterator Iterator[T]) {
	if n == nil {
		return
	}

	inOrder(n.Left, iterator)
	iterator(n.Value)
	inOrder(n.Right, iterator)
}

func inOrderReverse[T any](n *Node[T], iterator Iterator[T]) {
	if n == nil {
		return
	}

	inOrderReverse(n.Right, iterator)
	iterator(n.Value)
	inOrderReverse(n.Left, iterator)
}

func preOrder[T any](n *Node[T], iterator Iterator[T]) {
	if n == nil {
		return
	}

	iterator(n.Value)
	preOrder(n.Left, iterator)
	preOrder(n.Right, iterator)
}

func preOrderReverse[T any](n *Node[T], iterator Iterator[T]) {
	if n == nil {
		return
	}

	iterator(n.Value)
	preOrderReverse(n.Right, iterator)
	preOrderReverse(n.Left, iterator)
}

func postOrder[T any](n *Node[T], iterator Iterator[T]) {
	if n == nil {
		return
	}

	postOrder(n.Left, iterator)
	postOrder(n.Right, iterator)
	iterator(n.Value)
}

func postOrderReverse[T any](n *Node[T], iterator Iterator[T]) {
	if n == nil {
		return
	}

	postOrderReverse(n.Right, iterator)
	postOrderReverse(n.Left, iterator)
	iterator(n.Value)
}

//func Splay[T any](n *Node[T], val T) (*Node[T], bool) {
//	return nil, false
//}

// find the node with value val in tree t, return the node
func (t *Splay[T]) find(val T) (_ *Node[T]) {
	curr := t.root
	for curr != nil {
		if t.less(val, curr.Value) {
			curr = curr.Left
		} else if t.less(curr.Value, val) {
			curr = curr.Right
		} else {
			return curr
		}
	}
	return nil
}

func (t *Splay[T]) ascendLessThan(n *Node[T], pivot T, iterator Iterator[T]) bool {
	if n == nil {
		return true
	}

	if !t.ascendLessThan(n.Left, pivot, iterator) {
		return false
	}
	if t.less(n.Value, pivot) {
		if !iterator(n.Value) {
			return false
		}
		return t.ascendLessThan(n.Right, pivot, iterator)
	}

	return true
}

func (t *Splay[T]) ascendGreaterOrEqual(n *Node[T], pivot T, iterator Iterator[T]) bool {
	if n == nil {
		return true
	}

	if t.less(n.Value, pivot) {
		if !t.ascendGreaterOrEqual(n.Left, pivot, iterator) {
			return false
		}

		if !iterator(n.Value) {
			return false
		}
	}

	return t.ascendGreaterOrEqual(n.Right, pivot, iterator)

}

func (t *Splay[T]) ascendRange(n *Node[T], start, end T, iterator Iterator[T]) bool {
	if n == nil {
		return true
	}
	if t.less(end, n.Value) {
		return t.ascendRange(n.Left, start, end, iterator)
	}

	if t.less(n.Value, start) {
		return t.ascendRange(n.Right, start, end, iterator)
	}

	if !t.ascendRange(n.Left, start, end, iterator) {
		return false
	}

	if !iterator(n.Value) {
		return false
	}
	return t.ascendRange(n.Right, start, end, iterator)
}

func (t *Splay[T]) descendGreaterThan(n *Node[T], pivot T, iterator Iterator[T]) bool {
	if n == nil {
		return true
	}

	if !t.descendGreaterThan(n.Right, pivot, iterator) {
		return false
	}

	if t.less(pivot, n.Value) {
		if !iterator(n.Value) {
			return false
		}
		return t.descendGreaterThan(n.Left, pivot, iterator)
	}

	return true
}

func (t *Splay[T]) descendLessOrEqual(n *Node[T], pivot T, iterator Iterator[T]) bool {
	if n == nil {
		return true
	}

	if t.less(n.Value, pivot) {
		if !t.descendLessOrEqual(n.Right, pivot, iterator) {
			return false
		}

		if !iterator(n.Value) {
			return false
		}
	}

	return t.descendLessOrEqual(n.Left, pivot, iterator)
}

func (t *Splay[T]) descendRange(n *Node[T], start, end T, iterator Iterator[T]) bool {
	if n == nil {
		return true
	}

	if t.less(n.Value, start) {
		return t.descendRange(n.Right, start, end, iterator)
	}

	if t.less(end, n.Value) {
		return t.descendRange(n.Left, start, end, iterator)
	}

	if !t.descendRange(n.Right, start, end, iterator) {
		return false
	}

	if !iterator(n.Value) {
		return false
	}

	return t.descendRange(n.Left, start, end, iterator)
}

func (t *Splay[T]) splay(n *Node[T], val T) *Node[T] {
	if n == nil {
		return n
	}

	if t.less(val, n.Value) {
		if n.Left == nil {
			return n
		}

		if t.less(val, n.Left.Value) { //zig-zig
			n.Left.Left = t.splay(n.Left.Left, val)
			n = rotateRight(n)
		} else if t.less(n.Left.Value, val) { //zig-zag
			n.Left.Right = t.splay(n.Left.Right, val)
			if n.Left.Right != nil {
				n.Left = rotateLeft(n.Left)
			}
		}

		if n.Left != nil {
			n = rotateRight(n)
		}

	} else if t.less(n.Value, val) {
		if n.Right == nil {
			return n
		}

		if t.less(val, n.Right.Value) {
			n.Right.Left = t.splay(n.Right.Left, val)

			if n.Right.Left != nil {
				n.Right = rotateRight(n.Right)
			}
		} else if t.less(n.Right.Value, val) {
			n.Right.Right = t.splay(n.Right.Right, val)

			n = rotateLeft(n)
		}

		if n.Right != nil {
			n = rotateLeft(n)
		}

	}

	return n
}

//func (t *Splay[T]) insertNoReplace(n *Node[T], val T) *Node[T] {
//
//	if n == nil {
//		return newNode(val)
//	}
//
//	if t.less(val, n.Value) {
//		n.Left = t.insertNoReplace(n.Left, val)
//	} else {
//		n.Right = t.insertNoReplace(n.Right, val)
//	}
//
//	return n
//
//}

func (t *Splay[T]) replaceOrInsert(val T) (_ *Node[T], replaced *T) {

	if t.root == nil {
		return newNode(val), nil
	}

	t.root = t.splay(t.root, val) //val is at root now

	if t.less(val, t.root.Value) {
		n := newNode(val)
		n.Left = t.root.Left
		n.Right = t.root
		t.root.Left = nil
		t.root = n

	} else if t.less(t.root.Value, val) {

		n := newNode(val)
		n.Right = t.root.Right
		n.Left = t.root
		t.root.Right = nil
		t.root = n

	} else {
		replaced, t.root.Value = &t.root.Value, val
	}

	return t.root, replaced

}

func (t *Splay[T]) delete(val T) (deleted *T) {

	if t.root == nil {
		return
	}

	t.root = t.splay(t.root, val) //val is at root now

	if t.less(val, t.root.Value) || t.less(t.root.Value, val) {
		return nil
	} else {
		deleted = &t.root.Value

		if t.root.Left == nil {
			t.root = t.root.Right
		} else {
			x := t.root.Right
			t.root = t.root.Left
			t.root = t.splay(t.root, val)
			t.root.Right = x

		}
	}

	return

	//return nil, deleted
}

func (t *Splay[T]) print() {

	str := "SplayTree\n"
	if t.root == nil {
		fmt.Println("nil")
		return
	}

	output(t.root, "", true, &str)
	fmt.Println(str)
}

// -----------------------------------------public methods-----------------------------------------

// Len returns the number of nodes in the tree
func (t *Splay[T]) Len() int {
	return t.count
}

// Get returns the value of the node with value val
func (t *Splay[T]) Get(val T) (_ T, _ bool) {
	n := t.find(val)
	if n != nil {
		return n.Value, true
	}
	return
}

// Has returns true if the tree contains a node with value val
func (t *Splay[T]) Has(val T) bool {
	_, ok := t.Get(val)
	return ok
}

// Max returns the maximum value in the tree
func (t *Splay[T]) Max() (T, bool) {
	if t.root == nil {
		return *new(T), false
	}
	return findMax(t.root).Value, true
}

// Min returns the minimum value in the tree
func (t *Splay[T]) Min() (T, bool) {
	if t.root == nil {
		return *new(T), false
	}
	return findMin(t.root).Value, true
}

func (t *Splay[T]) AscendLessThan(pivot T, iterator Iterator[T]) {
	t.ascendLessThan(t.root, pivot, iterator)
}

func (t *Splay[T]) AscendGreaterOrEqual(pivot T, iterator Iterator[T]) {
	t.ascendGreaterOrEqual(t.root, pivot, iterator)
}

func (t *Splay[T]) AscendRange(start, end T, iterator Iterator[T]) {
	t.ascendRange(t.root, start, end, iterator)
}

func (t *Splay[T]) DescendGreaterThan(pivot T, iterator Iterator[T]) {
	t.descendGreaterThan(t.root, pivot, iterator)
}

func (t *Splay[T]) DescendLessOrEqual(pivot T, iterator Iterator[T]) {
	t.descendLessOrEqual(t.root, pivot, iterator)
}

func (t *Splay[T]) DescendRange(start, end T, iterator Iterator[T]) {
	t.descendRange(t.root, start, end, iterator)
}

//func (t *Splay[T]) InsertNoReplace(val T) {
//	t.count++
//	t.root = t.insertNoReplace(t.root, val)
//}

func (t *Splay[T]) ReplaceOrInsert(val T) (_ T, _ bool) {
	var replaced *T

	t.root, replaced = t.replaceOrInsert(val)

	if replaced == nil {
		t.count++
		return
	}
	return *replaced, true
}

func (t *Splay[T]) Delete(val T) (_ T, _ bool) {

	var replaced *T
	replaced = t.delete(val)

	if replaced != nil {
		t.count--
		return *replaced, true // found and deleted
	}
	return
}

func (t *Splay[T]) DeleteMax() (T, bool) {
	//temporary solution
	var zero T
	if t.root == nil {
		return zero, false
	}
	max := findMax(t.root)
	t.Delete(max.Value)
	return max.Value, true
}

func (t *Splay[T]) DeleteMin() (T, bool) {
	var zero T
	if t.root == nil {
		return zero, false
	}
	min := findMin(t.root)
	t.Delete(min.Value)
	return min.Value, true
}

func (t *Splay[T]) InOrder(iterator Iterator[T], reverse bool) {
	if reverse {
		inOrderReverse(t.root, iterator)
	} else {
		inOrder(t.root, iterator)
	}

}

func (t *Splay[T]) PreOrder(iterator Iterator[T], reverse bool) {
	if reverse {
		preOrderReverse(t.root, iterator)
	} else {
		preOrder(t.root, iterator)
	}
}

func (t *Splay[T]) PostOrder(iterator Iterator[T], reverse bool) {
	if reverse {
		postOrderReverse(t.root, iterator)
	} else {
		postOrder(t.root, iterator)
	}
}

func output[T any](node *Node[T], prefix string, isTail bool, str *string) {
	if node.Right != nil {
		newPrefix := prefix
		if isTail {
			newPrefix += "│   "
		} else {
			newPrefix += "    "
		}
		output(node.Right, newPrefix, false, str)
	}
	*str += prefix
	if isTail {
		*str += "└── "
	} else {
		*str += "┌── "
	}
	*str += node.String() + "\n"
	if node.Left != nil {
		newPrefix := prefix
		if isTail {
			newPrefix += "    "
		} else {
			newPrefix += "│   "
		}
		output(node.Left, newPrefix, true, str)
	}
}

func print[T any](node *Node[T]) {
	str := "SplayTree\n"
	if node == nil {
		fmt.Println("nil")
		return
	}

	output(node, "", true, &str)
	fmt.Println(str)
}
